...
- Name: e2e\ui\features\playgamepage.feature.spec.js >> PlayGame Page >> Player 1 HP bar shows correct initial value
- Location: .features-gen\e2e\ui\features\playgamepage.feature.spec.js:52:7
...
Locator: locator('[data-player=\'player1\']').locator('text=\'100 HP\'')
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('[data-player=\'player1\']').locator('text=\'100 HP\'')
...
> 83  |   await expect(page.locator(selector)).toBeVisible();
  ...
  166 | 
  167 | Then(
  168 |   "the word history shows damage {int} for {string}",
  169 |   async ({ page }, damage, word) => {
  170 |     const entry = page
  171 |       .locator("[data-word-history] [data-word-entry]")
  172 |       .filter({ hasText: word });
  173 | 
  174 |     await expect(entry).toHaveAttribute("data-damage", `${damage}`);
  175 |   }
  176 | );
  177 | 
```